...
std::vector<std::vector<float>> readMatrixFromFile(const std::string& filename, int& n) {
...
int main(int argc, char* argv[]) {
    ...
    std::string kernel_file_path = argv[6];

    int n;
    std::vector<std::vector<float>> mat = readMatrixFromFile(kernel_file_path, n);
    Mat kernel(n, n, CV_32F);
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            kernel.at<float>(i, j) = static_cast<float>(mat[i][j]);
        }
    } 
